...
class AsyncQuerier:
    ...
    async def create_user(self, *, user_id: builtins.int, user_name: builtins.str) -> typing.Optional[db.model.UsersRecord]:
        async with self.__pool.acquire() as conn:
            row = await conn.fetchrow('INSERT INTO "users" ("id", "name") VALUES ($1, $2) RETURNING "id", "name", "created_at"', user_id, user_name)
        return db.model.UsersRecord(id=row[0], name=row[1], created_at=row[2]) if row is not None else None

    async def get_user_by_id(self, *, user_id: builtins.int) -> typing.Optional[db.model.UsersRecord]:
        async with self.__pool.acquire() as conn:
    ...
    async def search_users_by_name_ilike(self, *, limit: builtins.int, user_name_pattern: builtins.str) -> typing.Sequence[db.model.UsersRecord]:
        async with self.__pool.acquire() as conn:
        ...
        return [db.model.UsersRecord(id=row[0], name=row[1], created_at=row[2]) for row in rows]